...
**Known issue** — Prowlarr's only audio-capable indexer (BitSearch) is dead: domain moved `bitsearch.to`→`bitsearch.eu` and it fails to connect, so **Lidarr has no working music indexer** until BitSearch is fixed/replaced with a working music indexer in Prowlarr (it then auto-syncs to Lidarr).
